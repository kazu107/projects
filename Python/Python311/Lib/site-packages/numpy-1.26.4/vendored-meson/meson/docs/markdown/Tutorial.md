...
//
static void activate(GtkApplication* index, gpointer user_data)
{
  GtkWidget *window;
  GtkWidget *label;

  window = gtk_application_window_new (index);
...
int main(int argc, char **argv)
{
  GtkApplication *index;
  int status;

  index = gtk_application_new(NULL, G_APPLICATION_FLAGS_NONE);
  g_signal_connect(index, "activate", G_CALLBACK(activate), NULL);
  status = g_application_run(G_APPLICATION(index), argc, argv);
...
If your index needs to use multiple libraries, you need to use separate
